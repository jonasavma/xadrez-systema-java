...
import xadrez.chess.pieces.Bishop;
import xadrez.chess.pieces.King;
import xadrez.chess.pieces.Knight;
import xadrez.chess.pieces.Pawn;
import xadrez.chess.pieces.Queen;
...
public class ChessMatch {
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("The chosen piece can 't move to target posiotn");
		}
	}
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			Position targtT = new Position(source.getRow(), source.getColumn() + 1);

			ChessPiece rook = (ChessPiece) board.removePiece(sourceT);
			board.placePiece(rook, targtT);
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			ChessPiece rook = (ChessPiece) board.removePiece(targtT);
	...
	private void validateSourcePosition(Position position) {
		...
		if (currentPlayer != ((ChessPiece) board.piece(position)).getColor()) {
			...
			throw new ChessException("The chosen piece is not yors!");
